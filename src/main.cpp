...
int main() {
  ...
  h.onMessage([&map_waypoints_x,&map_waypoints_y,&map_waypoints_s,
	...
	//std::cout << "Message Length " << length << " data[0] " << data[0] << " data[1] " << data[1] << std::endl;
    ...
    if (length && length > 2 && data[0] == '4' && data[1] == '2') {
      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
		//std::cout << "Telemetry Event" << std::endl;
		...
		// Define the actual (x, y) points will be used for the planner
		  ...
		  //START : Code for a path made up of (x,y) points that the car will visit sequentially every .02 seconds
		  ...
		  //END : Code for a path made up of (x,y) points that the car will visit sequentially every .02 seconds
